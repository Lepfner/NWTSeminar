    ...
    const fetchProductData = async () => {
      try {
        const response = await axios.get(
          `/chocolate/${window.location.href.slice(30, 65)}`
        );
          ...
          <div className="w-full animate__animated animate__fadeInDown animate__delay-1s">
            <div className="w-full rounded-xl opacity-90 hover:opacity-100 mt-20 flex flex-row align-center">
              ...
              <div className="flex flex-col items-start">
                <h1 className="text-3xl text-white">{productData.name}</h1>
                <h1 className="text-3xl text-white">{productData.price}</h1>
                <h1 className="text-3xl text-white">
                  {productData.description}
                </h1>
                <h1 className="text-3xl text-white">{productData.type}</h1>
                <h1 className="text-3xl text-white">
                  Ingredients: {productData.ingredients}
                </h1>
                <h1 className="text-3xl text-white">
                  Manufacturer: {productData.manufacturerName}
                </h1>
                {isAdmin && (
                  <div>
